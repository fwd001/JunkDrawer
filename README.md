...
**在线访问：<https://fwd001.github.io/JunkDrawer/>**（push 到 main 后由 Actions 自动发布）
...
1. 仓库 Settings → Pages → Source 选 **GitHub Actions**（没选的话 `configure-pages` 会报 “Get Pages site failed”）
2. 访问地址即 <https://fwd001.github.io/JunkDrawer/>
...
「今天/明天」的算法：出发时间是**一个绝对时刻**，只有它真的过去了才按整天往前滚——所以凌晨 00:30 打开时，昨天下午配的「明早 9 点」仍然显示成「今天 09:00」，不会又推一天。开始充电时间是**相对出发日的习惯**（出发当天 01:00 / 出发前夜 22:00 / 立即），跟着出发日一起走；那个点已经过了就塌缩成「立即」并注明原因。界面上的时间只在不是今天时才带「明天/后天」前缀，时间轴上只在跨天的位置标一次。
...
算法在 `src/tools/chargefit/calc.ts`：电流→功率→时长是解析式的，从小到大取第一个能卡进截止点的档位，所以「最温和」和「刚好来得及」是同一个解。24 条单测：`calc.spec.ts` 覆盖电流选择、档位对齐、电流不足、开始时间过晚和边界；`depart.spec.ts` 覆盖跨午夜、凌晨 00:30 仍算「今天」、原定时间已过塌缩成立即、以及「今天」前缀的省略规则。
